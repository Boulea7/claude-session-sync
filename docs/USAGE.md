...
> **Optional fields:** `description`, `created_at`, and `updated_at` are optional. Only `codex_session_id` and/or `gemini_session_id` are required for session continuity. The minimum valid entry is just `{ "codex_session_id": "<id>" }`.
>
> **可选字段：** `description`、`created_at`、`updated_at` 均为可选字段。仅 `codex_session_id` 和/或 `gemini_session_id` 是会话连续性所必需的。
